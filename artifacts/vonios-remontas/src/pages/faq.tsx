    ...
    <Layout>
      ...
      <div className="pt-32 pb-24 bg-background">
        <div className="container mx-auto px-6 max-w-3xl">
          <div className="mb-16">
            <p className="text-[10px] uppercase tracking-[0.25em] text-primary font-sans mb-4">D.U.K.</p>
            <h1 className="text-4xl md:text-5xl font-serif mb-4">Klausimai &amp; Atsakymai</h1>
            <p className="text-lg text-muted-foreground">
              Atvirai ir aiškiai apie vonios remonto procesą, kainas ir reikalavimus.
            </p>
          </div>

          <Accordion type="single" collapsible className="w-full mb-24 space-y-0 divide-y divide-white/5">
            {faqs.map((faq, i) => (
              <AccordionItem key={i} value={`item-${i}`} className="border-0 border-b border-white/5">
                <AccordionTrigger className="text-left text-base font-sans font-medium text-foreground py-5 hover:text-primary transition-colors [&>svg]:text-primary">
                  {faq.q}
                </AccordionTrigger>
          ...
          {/* Contact section */}
          <div id="susisiekti" className="border border-white/8 bg-[#111111] p-8 md:p-12 scroll-mt-24">
            <h2 className="text-2xl font-serif mb-2">Nerandate atsakymo?</h2>
            <p className="text-muted-foreground text-sm mb-8">
              Užpildykite formą ir susisieksiu su jumis asmeniškai.
            </p>
            <ContactForm />
          </div>
        </div>
